class OptionsController < ApplicationController
  ...
  def create
    ...
    # ta da - now we have an array of available options!

    # p @available_options
    # p '*' * 50
    # p @available_options[0][:start].first
    # p @available_options[0][:start].last
    # p (Time.strptime(@available_options[0][:start].last, "%a, %b %d %I:%M %P") + 15.minutes).strftime("%a, %b %d %I:%M %P")
    # # p Time.now
    # # p Time.now.strftime("%a, %b %d %I:%M %P")
    # # p Time.strpTime.now.strftime("%a, %b %d %I:%M %P")
    # # p Time.now + 15.minutes
